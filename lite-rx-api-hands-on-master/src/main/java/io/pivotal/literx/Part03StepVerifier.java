 ...
 * @see <a
 *     href="https://projectreactor.io/docs/test/release/api/reactor/test/StepVerifier.html">StepVerifier
 *     Javadoc</a>
 */
public class Part03StepVerifier {
  ...
  void expectSkylerJesseComplete(Flux<User> flux) {
    StepVerifier.create(flux)
        .expectNextMatches(user -> "swhite".equalsIgnoreCase(user.getUsername()))
        .expectNextMatches(user -> "jpinkman".equalsIgnoreCase(user.getUsername()))
        .expectComplete();
  }
  ...
  void expect10Elements(Flux<Long> flux) {
    StepVerifier.create(flux).expectNextCount(10).expectComplete();
  }
